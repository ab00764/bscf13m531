...
function initialize() {
    ...
    var map;
    //lat lng
    myLatlng = new google.maps.LatLng(31.5546, 74.3572);
    //define style
    var styles = [
        {
            //set all color
    ...
    var mapOptions = {
        zoom: 16,
        center: myLatlng,
        mapTypeControlOptions: { mapTypeIds: [google.maps.MapTypeId.HYBRID, 'map_style'] },
        zoomControl: true, //hide zoomContro
    }
    //adding attribute value
    map = new google.maps.Map(document.getElementById('templatemo_map'), mapOptions);
    ...
    function placeMarker(location) {
        ...
        google.maps.event.addListener(marker, "click", function (e) {
            ...
            content += '</br><form action="/home/Save" method="post" enctype="multipart/form-data">' +
            'Upload Image : <input type="file" name="img" /><input type="hidden" name="lat" value="' + location.lat() + '"/><input type="hidden" name="lng" value="' + location.lng() + '"/><input type="submit" value="Upload" />' +
            '</form>';
            content += "<br /><input type = 'button' align='center' onclick = 'DeleteMarker(" + marker.id + ")' value = 'Delete' />";
    ...
    // add info winodw
    ////var message ='<button name="remove-marker" class="remove-marker" title="Remove Marker">Remove Marker</button>'
    //var message = '<button name="remove-marker" class="remove-marker" title="Remove Marker">Remove Marker</button>'

    //var removeBtn = document.getElementById('remove-marker');
    ////google.maps.event.addListener(removeBtn, 'click', function () {
